...
final int[] notes= {0,2,4,7,9, 7, 9, 11, 14, 16, 14, 16};
...
final static int gridX = 3; // Grid X
final static int gridY = 3; // Grid Y
...
void setup() {
  ...
  movie = new Movie(this, sketchPath("../videos/out71.mp4"));
  ...
  for (int i = 0; i < gridX; i ++) {
    for (int j = 0; j < gridY; j ++) {
      ...
      if(abs(red(lastColor[i][j]) - mr) >  threshold){
        out.playNote(0, 0.1, new ToneInstrument(pentatonicTable[mr%pentatonicTable.length], 0.3));
      }
      if(abs(red(lastColor[i][j]) - mr) >  threshold){
        out.playNote(0, 0.1, new ToneInstrument(pentatonicTable[mg%pentatonicTable.length], 0.3));
      }
      if(abs(red(lastColor[i][j]) - mr) >  threshold){
        out.playNote(0, 0.1, new ToneInstrument(pentatonicTable[mb%pentatonicTable.length], 0.3));
      }
